...
export function Topbar({ onOpenSidebar }) {
    ...
    return (
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-md sm:px-6">
            ...
            {/* Global search (non-functional placeholder for Phase 1) */}
            <div className="relative hidden max-w-md flex-1 sm:block">
                <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                <input
                    type="search"
                    disabled
                    placeholder="Search parts, vouchers, work orders…"
                    aria-label="Global search (available next phase)"
                    className="h-10 w-full cursor-not-allowed rounded-lg border border-input bg-muted/40 pl-9 pr-3 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none"
                />
            </div>
